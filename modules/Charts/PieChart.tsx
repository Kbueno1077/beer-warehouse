...
function PieChart({serverFetchedBeers}: PieChartsProps) {
    ...
    const getOption = () => {
        return {
            ...
            legend: {
                ...
                bottom: 'bottom',
                textStyle: {
                    color: theme === DARK_MODE ? 'white' : '#333333',
                }
            },

            emphasis: innerWidth >= 768 ? {
                label: {formatter: '{b}: {d}%',},
                itemStyle: {
                    shadowBlur: 10,
                    shadowOffsetX: 0,
    ...
    function keyPieChart(array: Array<BeerType>, keyAttr: string) {
        ...
        const data = Object.keys(result).map((key) => {
            if (keyAttr === 'country') {
                return {value: result[key], name: key !== "TBD" ? tcountries(countryCodes[key]) : "TBD"}
            }
        ...
        <>
            <div className='w-full flex justify-center md:justify-start'>
                <ButtonGroup variant='bordered' color='primary'>
                    <Button variant={attribute === 'country' ? 'solid' : 'bordered'}
                            onClick={() => changeAttribute('country')}>{t('country')}</Button>
                    <Button variant={attribute === 'ml' ? 'solid' : 'bordered'}
                            onClick={() => changeAttribute('ml')}>{t('ml')}</Button>
                    <Button variant={attribute === 'alcohol_percentage' ? 'solid' : 'bordered'}
                            onClick={() => changeAttribute('alcohol_percentage')}>{t('alcohol_percentage')}</Button>
            ...
            {attribute && <>
                ...
                <div
                    className='w-full flex justify-end items-center p-2 text-gray-500 italic'>{data.length} {t('instances')}
                </div>
            </>
            }
        </>
