...
const BeerCard = ({beer}: BeerCardProps) => {
    ...
    const [selectedKeys, setSelectedKeys] = useState<Iterable<any>>(new Set([]));
        ...
        <Card isFooterBlurred={true}>
            <CardHeader className="absolute z-10 top-1 flex justify-between items-start">
                <Chip variant="light" radius="sm" className={styles.chipName}>
                    <p className="text-tiny text-white/75 uppercase font-bold">{beer.name}
                    </p>
                </Chip>
            </CardHeader>
            ...
            <CardFooter className="absolute bg-white/30 bottom-0 border-t-1 border-zinc-100/50 z-10 flex-col">
                <div className='w-full flex align-middle items-center justify-between'>
                    <div>
                        <p className="text-black text-tiny">{beer.alcohol_percentage >= 0 ? `${t('abv')}: ${beer.alcohol_percentage} %` : t('abv-na')}</p>
                ...
                {user && user.role === ADMIN_ROLE &&
                    <Accordion
                        ...
                        <AccordionItem indicator={({isOpen}) => (isOpen ? <div></div> : <div></div>)}
                                       key="1"
                                       classNames={{trigger: styles.wrapper}}
                                       aria-label="Beer Actions"
                                       startContent={
                                           <div
                                               style={{transform: (selectedKeys as Set<any>).size ? 'rotate(180deg)' : ""}}
