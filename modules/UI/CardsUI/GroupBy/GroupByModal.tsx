...
export default function GroupByModal() {
    ...
    return (
        <>
            <Button color='primary' onPress={onOpen} variant='faded' aria-label={t('btn-aria')}>
                <PiSortAscendingBold className='w-4 h-4'/> {t('title')} {groupBy !== "" ? " *" : ""}
            </Button>

            <Modal size='2xl' isOpen={isOpen} onClose={handleClose} onOpenChange={onOpenChange}>
                <ModalContent>
                    {() => (
                        <>
                            <ModalHeader className="flex flex-row gap-1">{t('title')}</ModalHeader>
                            <ModalBody className="flex flex-row justify-center gap-2 flex-wrap">

                                <GroupByComponent handleGroupBy={handleGroupBy} selectedGroupBy={selectedGroupBy}/>


                            </ModalBody>
                            <ModalFooter>
                                <Button color="danger" variant="light" onPress={() => cancelGroups()}>
                                    {t('cancel')}
                                </Button>
                                <Button color="primary" onPress={() => applyGroups()}>
                                    {t('apply')}
                                </Button>
                            </ModalFooter>
                        </>
