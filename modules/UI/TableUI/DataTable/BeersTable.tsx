...
export default function BeersTable() {
    ...
    const filteredItems = React.useMemo(() => {
        ...
        if (hasSearchFilter) {
            filteredBeers = filteredBeers.filter((beer) => {
                    return !!(beer && beer.name && beer.name.toLowerCase().includes(filters.search.toLowerCase()));
                }
            )
        }
        ...
        if (filters?.alcoholFilters?.length !== alcoholOptions.length) {
            filteredBeers = filteredBeers.filter((beer) => {
                    return filters.alcoholFilters.includes(mapPercentToString(beer.alcohol_percentage))
                }
            );
        }
        ...
        if (filters?.mlFilters?.mlValue !== -1) {
            filteredBeers = filteredBeers.filter((beer) => {
                    return mapOperands(filters.mlFilters.operand, filters.mlFilters.mlValue, beer.ml)
                }
            );
        }
        ...
        if (filters.countryFilters.length) {
            filteredBeers = filteredBeers.filter((beer) =>
                filters.countryFilters.map((countryCode) => countryCode.value).includes(beer.country)
            );
        }

        setPage(1);
        return filteredBeers;
    }, [allBeers, filters.countryFilters.length, filters.search, filters.mlFilters.operand, filters.mlFilters.mlValue, filters.alcoholFilters]);
    ...
    const sortedItems = React.useMemo(() => {
        ...
        return [...filteredItems].sort((a: BeerType, b: BeerType) => {
            const first = a[sortDescriptor.column as keyof BeerType] as number;
            const second = b[sortDescriptor.column as keyof BeerType] as number;
    ...
    const renderCell = React.useCallback((beer: BeerType, columnKey: React.Key) => {
        const cellValue = beer[columnKey as keyof BeerType];

        switch (columnKey) {
            case "name":
                const imageSplit = beer.evidence_img ? beer.evidence_img.split('upload') : ["", ""]
                const thumbnail = imageSplit[0] + "upload/c_thumb,w_200,g_face" + imageSplit[1]

                return (
                    <User
                        avatarProps={{radius: "full", size: "sm", src: thumbnail || ""}}
                        classNames={{
                ...
                return (
                    <div className="flex items-center align-middle gap-2">
                        {cellValue !== "TBD" && cellValue !== "" &&
                            <FlagAvatar value={typeof cellValue !== 'string' ? "" : cellValue}/>
                        }

                        {cellValue !== "TBD" && cellValue !== "" &&
                            //@ts-ignore
                            <p className="text-bold text-small capitalize">{cellValue ? tcountries(countryCodes[cellValue]) ? tcountries(countryCodes[cellValue]) : countryCodes[cellValue] : ""}</p>
                        }
                    </div>
                )
                    ;
            ...
            case "alcohol_percentage":
                const mapColor = mapPercentToString(beer.alcohol_percentage)
            ...
            case "evidence_img":
                ...
                return (
                    <>
                        {url &&
                            <Button color="primary" onClick={() => window.open(url, "_blank")} size='sm'
                                    variant="bordered" startContent={<PiImagesSquare/>}>
                                {t('evidence')}
                            </Button>
                        }
                    </>
                    ...
                    ;
            case "actions" :
                const currentBeer = allBeers.find((item) => item.id === beer.id)

                return (
                    <div className="relative flex justify-end items-center gap-2">
                        <Button disabled={true} variant="faded" size={"sm"} isIconOnly color="danger"
            ...
            <div className="flex flex-col gap-4">
                <div className="flex flex-col justify-between gap-3 sm:flex-row">
                    <Input
                        ...
                        variant="bordered"
                        onClear={() => changeFilters({...filters, search: ""})}
                            ...
                            >
                                {columns.map((column) => (
                                    <DropdownItem key={column.uid} className="capitalize">
                                        {capitalize(t(column.name.toLowerCase()))}
                                    </DropdownItem>
                                ))}
                            </DropdownMenu>
                        </Dropdown>
        ...
        >
            <TableHeader columns={headerColumns}>
                {(column) => (
                    ...
                    >
                        <div className={`flex ${column.uid === 'actions' && 'justify-end'}`}>
            ...
            <TableBody
                emptyContent={hasSearchFilter || filters.alcoholFilters || filters.countryFilters || filters.mlFilters ? 'No beers match applied filters' : "No beers found"}
                items={sortedItems}>
                {(item) => (
