    ...
    if (hasSearchFilter) {
      filteredBeers = filteredBeers.filter((beer) => {
        return !!(
          beer &&
          beer.name &&
          beer.name.toLowerCase().includes(filters.search.toLowerCase())
        );
      });
    }
      ...
      switch (columnKey) {
        case "name":
          const imageSplit = beer.evidence_img
            ...
            : ["", ""];
          const thumbnail =
            imageSplit[0] + "upload/c_thumb,w_200,g_face" + imageSplit[1];
    ...
    >
      <TableHeader columns={headerColumns}>
        {(column) => (
          ...
          >
            <div
              className={`flex ${column.uid === "actions" && "justify-end"}`}
            >
