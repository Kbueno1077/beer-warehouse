...
import {Button, CheckboxGroup, Chip, tv, useCheckbox, VisuallyHidden} from "@nextui-org/react";
...
export default function AlcoholFilter({handleFilters}: any) {
    ...
    const [groupSelected, setGroupSelected] = React.useState(filters.alcoholFilters);
    ...
    const removeFilter = () => {
        ...
        handleFilters('alcoholFilters', ['soft', 'normal', 'high', 'extra', 'hardcore'])
    }
            ...
            >
                <div className='flex flex-wrap gap-1'>
                    {alcoholOptions.map(option =>
                        <CustomCheckbox key={option.uid} value={option.name}>{t(option.name)}</CustomCheckbox>)
                    }
                </div>
            </CheckboxGroup>
        </div>
