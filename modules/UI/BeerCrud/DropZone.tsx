...
function DropZone({zone, files, handleFiles, currentImg = ""}: DropZonePorps) {
    ...
    const {getRootProps, getInputProps} = useDropzone({
        ...
        onDrop: acceptedFiles => {
            setSeeImage(true)
            handleFiles(zone, acceptedFiles.map(file => Object.assign(file, {
        ...
        <div {...getRootProps()}
            ...
            {!seeImage &&
                <><MdOutlineSaveAlt className='w-8 h-8 mb-2'/>
                    <p><strong>{t('choose file')}</strong> {t('drag file')}</p>
                    <p>{zone === 'evidence_img' ? t('evidence-img') : t('preview-img')}</p>
                    {files.length > 0 && <p className='mt-2'>{t('save image')}</p>}
                </>
            }
                ...
                <>
                    {seeImage &&
                        <div className='relative w-full mx-auto'>
                            {files.length > 0 ? images : currentImg ? selectedBeerImg : ""}
                        </div>
                    }

                    <div className='position absolute right-2 bottom-2 z-20 '>
                        {!seeImage ?
                            <Button isIconOnly disableRipple variant='faded' onClick={toggleSeeImage}>
