...
export default function DeleteBeer({selectedBeer}: DeleteBeerProps) {
    ...
    const deleteBeer = async () => {
        ...
        try {
            const {data: {record, error}} = await axios.delete(`/api/beer`, {
                params: {
                    id,
                    evidence_public_id
                },
    ...
    return (
        <>
            {user &&
                <Button size={"sm"} onPress={onOpen} isIconOnly color="danger" aria-label={t('remove-aria')}>
            ...
            <Modal isOpen={isOpen} onOpenChange={onOpenChange}>
                <ModalContent>
                    {(onClose) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1">{t('confirm remove-title')}</ModalHeader>
                            <ModalBody>
                                <p>
