...
export async function checkSession() {
    ...
    throw ({error: true, response: {data: {errorMessage: "Log in to do this operation"}}})
}
...
export function groupByMethod(array: any, property: any = "") {
    let result = array.reduce((result: any, item: any) => {
        ...
        if (item[property]) {
            if (property === 'name') {
                key = item[property][0].match(/[0-9]/) ? '#' : item[property][0];
            } else if (property === 'country' && item[property] === 'TBD' && item[property] === "") {
